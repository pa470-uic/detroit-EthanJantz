...
This model trains on properties that sold in 2019 and tests on those that didn't.

```{r lm 4}
train <- lm_model_data_sf %>%
  filter(sale_year == 2019)
test <- lm_model_data_sf %>%
  filter(sale_year != 2019)
...
The following is a table of coefficients for our linear regression model. This shows the impact that each of the variables in our formula had on the final assessment of a property. The factors with the largest impact on assessed value were the style of the home and the percentage of Black/African-American population within the home's census tract. Both had positive coefficients.
